...
double heston_fft_price(double S0, double K, double v0, double eta, double alpha, double r, double rho, double kappa, double theta, double xi, double T, int N)
{
    ...
    if(log_strike <= log_strike_grid[0] || log_strike > log_strike_grid[N-1]){
    ...
    std::vector<double> call_price_grid(N);

    for(int u = 0; u < N; ++u){
        call_price_grid[u] = (exp(-alpha * log_strike_grid[u]) / pi) * out[u][0];
    }

    auto it = std::lower_bound(log_strike_grid.begin(), log_strike_grid.end(), log_strike);
    ...
    double call_price = lower_call_price + ((log_strike - log_strike_grid[idx - 1]) / lambda) * (upper_call_price - lower_call_price);
